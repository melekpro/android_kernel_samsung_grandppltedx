...
#   sudo rm -rf gcc/.git
    ...
    echo "${GRN} #                                   #"
    echo "${GRN} #####################################"
    git clone -q https://github.com/Joker152/Linaro-5.5.0
    export ARCH=arm CROSS_COMPILE=${PWD}/Linaro-5.5.0/bin/arm-linux-gnueabi-
    export SUBARCH=arm
else
    export ARCH=arm CROSS_COMPILE=${PWD}/Linaro-5.5.0/bin/arm-linux-gnueabi-
    export SUBARCH=arm
...
function BUILD() {
        ...
        echo "${PURP} #                                   #"
        echo "${PURP} #####################################"
        make O=BGH-Out/ TARGET_ARCH=arm mt6737t-org_defconfig | tee -a defconfig.log
        echo "${YLW} #####################################"
        echo "${YLW} #                                   #"
